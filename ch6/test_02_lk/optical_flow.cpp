        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    if (inverse == true) {
                        J(0) = -0.5*(GetPixelValue(img2,kp.pt.x + x+1,kp.pt.y + y) - GetPixelValue(img2,kp.pt.x + x -1,kp.pt.y + y));
                        J(1) = -0.5*(GetPixelValue(img2,kp.pt.x + x ,kp.pt.y + y+1) - GetPixelValue(img2,kp.pt.x + x ,kp.pt.y + y-1));
                    }
                    else {
                        // Inverse Jacobian
                        J(0) = -0.5 * (img1.at<uchar>(kp.pt.x+dx+x+1, kp.pt.y+dy+y)   - img2.at<uchar>(kp.pt.x+dx+x-1, kp.pt.y+dy+y));
                        J(1) = -0.5 * (img1.at<uchar>(kp.pt.x+dx+x,   kp.pt.y+dy+y+1) - img2.at<uchar>(kp.pt.x+dx+x,   kp.pt.y+dy+y-1));
...
void OpticalFlowMultiLevel(
    ...
    for(int level = pyramids-1;level>=0;level--)
    {
        ...
        /// workers.push_back(std::thread(work,pyr1[level], pyr2[level], kp1_pyr, kp2_pyr, success,false));
